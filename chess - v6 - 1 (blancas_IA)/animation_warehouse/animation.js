...
const emotionMap = {
    enojado: { start: 0, frames: 2 },
    hablar: { start: 2, frames: 2 },
    triste: { start: 4, frames: 2 },
    sorprendido: { start: 6, frames: 2 },
    llorando: { start: 8, frames: 2 }
};

function getFrameSize() {
    return sprite.clientWidth;
}

function setFrame(emotion, frame) {
    const data = emotionMap[emotion] || emotionMap.hablar;

    const size = getFrameSize();
    const x = -(data.start + frame) * size;
...
function startTalking() {
    ...
    talkingInterval = setInterval(() => {
        setFrame(currentEmotion, frameIndex % 2);
...
window.addEventListener("message", (e) => {
    ...
    if (ev.color !== "b") return; // solo reaccionar al jugador
...
window.addEventListener("message", (e) => {
    ...
    if (dificultad) {
        ...
        // 🔥 IMPORTANTE: notificar a union (como si el usuario hizo click)
