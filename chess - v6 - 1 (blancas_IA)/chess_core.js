...
let turn='w';
let playerColor='b'; // jugador controlará negras
...
function drawBoard(){
    ...
    const drawRows=[0,1,2,3,4,5,6,7];
    const drawCols=[0,1,2,3,4,5,6,7];

    // invertir tablero si jugador es negro
    if(playerColor==='b'){
        drawRows.reverse();
        drawCols.reverse();
    }

    for(let r of drawRows){
        for(let c of drawCols){
...
function handleClick(r,c){
    ...
    if(turn!==playerColor) return; // bloquea al turno contrario (ahora jugador negro)
...
// ⚡ Si el turno inicial es IA blanca, hacer primer movimiento automáticamente
console.log("CHESS_CORE: turno inicial =", turn, 
            "| requestBlackMove existe =", typeof window.requestBlackMove);