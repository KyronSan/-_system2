...
function getFrameSize() {
    return sprite.clientWidth; // obtiene tamaño REAL (120 o 240)
}

// posición de cada emoción en el spritesheet
const emotionMap = {
    enojado:      { row: 0, start: 0, frames: 2 },
    hablar:       { row: 0, start: 2, frames: 2 },
    triste:       { row: 0, start: 4, frames: 2 },
    sorprendido:  { row: 0, start: 6, frames: 2 },
    llorando:     { row: 0, start: 8, frames: 2 }
};

const sprite = document.getElementById("characterSprite");

function setFrame(emotion, frameIndex) {
    const data = emotionMap[emotion];
    if (!data) return;

    const size = getFrameSize();

    const x = (data.start + frameIndex) * size;
    const y = data.row * size;
...
function startTalking() {
    ...
    talkingInterval = setInterval(() => {
        ...
        setFrame(currentEmotion, frameIndex);
    }, 180);
}
